...
export const asignCandidateForm = (props) => {
  const { nro, nombre, eventList, setModalData, refreshEventList } = props;
...
export const removeCandidateForm = (props) => {
  const { nro, category, nombre, setModalData, refreshCanddiateList } = props;

  const content = (
    <>
      <p>
        ¿Desea desvincular <b>{nombre}</b>de esta lista?
      </p>
      <input type="hidden" name="candidato" defaultValue={nro} />
