...
export const insertEventForm = (props) => {
  const { setModalData, refreshEventList } = props;
  ...
  const content = (
    <>
      ...
      <label htmlFor="dia_elecciones">
        Día de las elecciones (DD-MM-YYY 00:00)
      </label>
      <input
        type="datetime-local"
        required
        ...
        id="dia_elecciones"
      />
      <label htmlFor="dia_elecciones_final">
        Final de las elecciones (DD-MM-YYY 00:00)
      </label>
      <input
        type="datetime-local"
        required
        name="dia_elecciones_final"
        id="dia_elecciones_final"
      />
...
export const updateEventForm = (props) => {
  ...
  const content = (
    <>
      ...
      <input
        defaultValue={inicio_elecciones}
        type="datetime-local"
        required
        name="dia_elecciones"
        id="dia_elecciones"
...
export const removeEventForm = (props) => {
  const { idEvent, event, setModalData, refreshEventList } = props;

  const content = (
    <>
      <input defaultValue={idEvent} type="hidden" name="nro_evento" />
      <p>
        ¿Estas seguro que quieres eliminar <b className="t-green">"{event}"</b>{" "}
        de forma permanente?
      </p>
    </>
  );
...
export const showEventDetails = (props) => {
  const { event, inicio, final, elecciones, finalElecciones, setModalData } =
    props;
  const content = (
    <>
      <p>
      ...
      <p>
        <b>Día de las elecciones: </b>
        {elecciones}
      </p>
      <p>
        <b>Final de las elecciones: </b>
        {finalElecciones}
      </p>
      <p>
        <b>Final del evento: </b>
        {final}
      </p>
    </>
  );
